...
const CODE_INTENT_WORDS = [
  ...
  'how to', 'how do i', 'sample', 'program', 'function', 'implement',
...
function findAnswer(question) {
  ...
  if (hasCodeIntent) {
    ...
    // User asked for code but no topic matched → show guidance
    return `**I can provide detailed, ready-to-run code for many GIS & Remote Sensing tasks!** 🛰️

Here are the code topics available — be specific and I'll give you the full script:

| Topic | Example question |
|-------|-----------------|
| 🌿 **NDVI (Python / GEE)** | *"Give me NDVI python code"* or *"GEE NDVI script"* |
| 🌊 **Flood Mapping (SAR)** | *"SAR flood detection code"* or *"flood mapping GEE"* |
| 🗺️ **LULC Classification** | *"Random forest classification python script"* |
| 📐 **Reproject Raster** | *"Batch reproject rasterio code"* |
| 🔷 **Vector / GeoPandas** | *"GeoPandas buffer dissolve script"* |
| 🌐 **Web Map (Leaflet)** | *"Leaflet HTML map code"* |
| 🛠️ **GDAL Commands** | *"GDAL reproject and clip commands"* |
| 🗄️ **PostGIS SQL** | *"PostGIS spatial query code"* |
| 📊 **Change Detection** | *"Change detection python script"* |
| ⛰️ **DEM / Terrain** | *"DEM slope hillshade python"* |
| 📡 **Full GIS Workflow** | *"Complete python geospatial code"* |

**Tip:** The more specific your question, the more accurate and detailed my code answer will be!`;
  }
...
const GIS_EXPERT_PROMPT = `You are an expert GIS (Geographic Information Systems) and Remote Sensing scientist and educator. You have deep knowledge in:

- Remote Sensing: satellite imagery, sensors (Sentinel, Landsat, MODIS, Radar/SAR), image processing, spectral indices
- GIS Analysis: spatial analysis, geostatistics, coordinate systems, projections, vector/raster operations
- Photogrammetry & UAV/Drone mapping
- Google Earth Engine (GEE) scripting and cloud-based RS
- Software: QGIS, ArcGIS, ENVI, ERDAS, SNAP
- Programming: Python (GDAL, Rasterio, GeoPandas, Shapely), R, JavaScript (Leaflet, OpenLayers)
- Applications: land use/cover, vegetation mapping, flood monitoring, agriculture, urban planning, disaster management
- DEM analysis, terrain modeling, LiDAR processing
- Change detection, time-series analysis

RESPONSE RULES:
1. Always give COMPLETE, detailed answers — never vague or one-liners
2. Structure your answer clearly with these sections when relevant:
   - **Overview** — what it is and why it matters
   - **Key Concepts** — important terms and principles
   - **Step-by-Step Process / Method** — detailed workflow
   - **Tools & Software** — what software/libraries to use
   - **Data Sources** — where to get the data
   - **Code Example** — Python/GEE/GDAL code when relevant
   - **Tips & Best Practices** — common mistakes and pro tips
3. Use bullet points, numbered steps, bold headings for clarity
4. Include real data sources (Copernicus, USGS, NASA, etc.)
5. Mention specific bands, indices, algorithms by name
6. If asked about software, give step-by-step instructions
7. If asked about code, provide complete working code
...
async function callGroqForGIS(question) {
  const apiKey = localStorage.getItem('spatialtech_groq_key');
...
async function sendMessage() {
  ...
  if (groqAnswer) {
    ...
    const hasKey = !!localStorage.getItem('spatialtech_groq_key');
    if (!hasKey) {
      addMessage(
